...
class WinAI_Endpoints
{
    ...
    public function __construct()
    {
        ...
        add_filter('rest_pre_serve_request', function ($served, $result, $request, $server) {
            header('X-Windesheim-Courses-version: ' . WinAI_API_VERSION);
            return $served;
        }, 10, 4);
    }
    ...
    {
        register_rest_route('winai/v1', '/courses', [
            'methods' => 'GET',
        ...
        register_rest_route('winai/v1', '/courses/(?P<id>\d+)', [
            'methods' => 'GET',
    ...
    public function create_course($request)
    {
        ...
        $course_weight = $wpdb->get_var("SELECT MAX(weight) FROM {$wpdb->prefix}WinAI_Courses");
        $course_weight = $course_weight == null ? 0 : $course_weight + 1;

        // Create WinAI_Courses
        $courses_table = $wpdb->prefix . 'WinAI_Courses';
        $course_data = [
            'title' => $content->title,
    ...
    public function update_course($request)
    {
        ...
        // Delete WinAI_Course_Stages
        $stages_table = $wpdb->prefix . 'WinAI_Course_Stages';
        $wpdb->delete($stages_table, ['course_id' => $id]);

        // Delete WinAI_Stage_Blocks
        $blocks_table = $wpdb->prefix . 'WinAI_Stage_Blocks';
